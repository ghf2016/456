...
#include <QMessageBox>
#include <QProcess>
IMeetingFrame* g_pMeetingFrame = NULL;      //这个MeetingFrame  应该是跟那个 meeting服务器连着的，meeting服务器又跟php那个连着？
                                             ///全局的
IZYMediaStreamManager *g_pStreamMgr = NULL; //流媒体？

EventCenter*EventCenter::m_instahce = NULL;
EventCenter* EventCenter::GetInstance()       //事件中心
{
	...
	: QWidget(parent, flags)
{
	...
	//初始化视频会议框架
	CreateMeetingFrame(&g_pMeetingFrame,this,(HWND)RoomWdg::GetInstance()->winId(),  //定义在IMeetingFramelmpl里面
...
{
	//qDebug()<<"On_MeetingEvent_GetRoomList";              //这个信号是自己定义的
	emit EventGetRoomList(total);               //这个是收到了 服务器发送的房间列表  发射信号！！！！！
}
//收到文本聊天消息
