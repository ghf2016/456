...
{                                                    //这个应该是通用的吧,s虽然我看的是用户 ，看错了
	//用户上线，插入用户列表
	if(sessionID == m_myUserInfo.sessionID)
		return;
	Mutex::ScopedLock lock(m_Mutex);         //这个算是那种 根据生存空间的锁吗？
	PClient_Online_User_Info pInfo = new Client_Online_User_Info();  //自己定义的数据格式
...
{
	MeetingConnImpl::GetInstance()->GetRoomList();  //这个还是转头调用的公用的
}
