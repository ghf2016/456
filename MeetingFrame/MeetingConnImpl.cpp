...
//得到房间列表
void MeetingConnImpl::GetRoomList()      //这个是通用的部分吧。。
{
	char *utfStr = new char[64];
	sprintf(utfStr, "{\"cmd\":\"getRoomList\"}\r\n");    //得到房间的命令，这个不会是模拟的json格式吧？
	FastMutex::ScopedLock autoLock(m_lock);              //还加了一个独占锁
	m_sendBufList.push(utfStr);                          //这个只是发送的部分吧？
}
//得到房间用户列表
...
void MeetingConnImpl::OnGetRoomUserList(Object::Ptr object)   //  object  是返回的数据的指针
{
	Var varStatus = object->get("status");                  //  这个json格式的数据，反正就是一个一个解析
	if(varStatus == 0)
	{                                                      //状态0 应该是 可以 正常
		Var resultObj = object->get("result");             //  result应该是想要的数据
		Poco::JSON::Array::Ptr array =resultObj.extract<Poco::JSON::Array::Ptr>();
		for(Poco::Int32 i = 0;i<array->size();i++)   //这个用的是什么指针？？
		{                                           //列表指针？
			Parser parser;   //解析             
			Var result;           //
			Var item = array->get(i);
			string strItem = item;                  //房间列表嘛，  item就是要买要的数据，一行的
			try
			{

				result = parser.parse(strItem);   //为什么还要解析？

			}
			catch(JSONException& jsone)
			{
				std::cout << jsone.message() << std::endl;        //这个json 不会也是poco封装过的吧。
				return;
			}

			Object::Ptr tempObj = result.extract<Object::Ptr>();   // 看错了 。。。。这个是 用户列表！！
			...
			string strUserName = varUserName;           //读取出来的数据还要转成相应的格式
...
void MeetingConnImpl::OnGetRoomList(Object::Ptr object)
{
	...
	if(varStatus == 0)
	{
		try
		{
			Var resultObj = object->get("result");//这个object指向一个完整的json，
			Poco::JSON::Array::Ptr array =resultObj.extract<Poco::JSON::Array::Ptr>();
			for(Poco::Int32 i = 0; i<array->size();i++)
			{
				Parser parser;   //这个读取数据应该是poco独特的方法。
				Var result;
				Var item = array->get(i);  
				string strItem = item;     // 这个item 应该也是一个完整的json数据
				result = parser.parse(strItem); //再解析
				...
				FastMutex::ScopedLock autoLock(m_lock);   //锁
				pRoomInfo->ulRoomID = varRoomID;          //把上面的数据保存到 info里面？
				...
				pRoomInfo->endTime = varEndTime;             //保存到结构体里面
				...
				m_pRoomList.push_back(vector<PROOM_INFO>::value_type(pRoomInfo));  //一个链表，保存到成员变量里面了
			}
			if(array->size()>0)
				m_pIMeetingEvent->On_MeetingEvent_GetRoomList(array->size()); //这个是循环完了
		}                                             // 传入的参数是  有几个？
		catch(Poco::Exception& exc)
		{
 ...
 PROOM_INFO MeetingConnImpl::GetRoomInfo(int index)
{
	...
	return  m_pRoomList[index];   //返回某一个房间的信息。。理论上也可以用这个来获取整个数据
	                                //整个项目只有这一个函数返回了 Roomlist
}
...
void MeetingConnImpl::OnGetRoomInfo(Object::Ptr object)   //这个和 有返回的不一样。。这个传入的是指针，不返回
{
	FastMutex::ScopedLock autoLock(m_lock);
	Var varStatus = object->get("status");
	int docIndex = 0;
...
void MeetingConnImpl::ProcessJsonCommand(char * buffer,Poco::Int32 nLen)   //处理收到的字符串
{
	Parser parser;                               //解析的
	Var result;
	//printf("\n %s len: %u\n",buffer,nLen);
	...
	try
	{
		result = parser.parse(buffer);  //尝试
	}
	catch(JSONException& jsone)
	{
		//printf("%s \n",buffer);
		std::cout << jsone.message() << std::endl;       //这个也应该是有问题的吧
		return;
	}
	catch(Poco::Exception& exc)
	{ 
		//printf("json string parse error, len: %u",nLen);  //出错
		//printf("%s \n",buffer);
		std::cout << exc.message() << std::endl;
		m_pIMeetingEvent->OnNetEvent(NET_EVENT_JSON_ERROR,"json parse error");
		return;
	}
	Object::Ptr object = result.extract<Object::Ptr>();   //这里才是成功的，
	Var varCommand = object->get("cmd");                  //为什么要用指针来指代数据
	std::string strCommand = varCommand;                  //Var 是poco的数据格式
	if(strCommand == "handSharke1_ret")
	{
		OnHandSharke1(object);                 //这个是在判断 cmd的返回值
	}
	...
	else if(strCommand == "getRoomUserList_ret")     //我们要找到命令 房间的列表
	{
		OnGetRoomUserList(object);                  //这里都还是根据 json命令个格式来 解析数据。
	}
